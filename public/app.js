...
const topPanelHandle = document.getElementById('top-panel-handle');
const legendPanel = document.getElementById('legend-panel');
const legendPanelToggle = document.getElementById('legend-panel-toggle');
const legendPanelHandle = document.getElementById('legend-panel-handle');
...
function attachPanelGestures(panel, toggle, handle, direction) {
  let startY = null;
  let startX = null;
  ...
  handle.addEventListener('pointerup', (event) => {
    ...
    if (isMostlyVertical) {
      if (direction === 'up' && deltaY < -24) {
        setPanelCollapsed(panel, toggle, true);
      } else if (direction === 'up' && deltaY > 24) {
        setPanelCollapsed(panel, toggle, false);
      } else if (direction === 'down' && deltaY > 24) {
        setPanelCollapsed(panel, toggle, true);
      } else if (direction === 'down' && deltaY < -24) {
        setPanelCollapsed(panel, toggle, false);
      }
...
async function geocodeAddress(address) {
  const isLikelyDevServer = Boolean(window.location.port);
  ...
  let response = await fetch(`${GEOCODER_URL}?${boundedParams.toString()}`, {
    cache: 'no-store',
  });
  ...
  if (results.length === 0) {
    ...
    response = await fetch(`${GEOCODER_URL}?${fallbackParams.toString()}`, {
      cache: 'no-store',
    });

    if (!response.ok) {
      throw new Error(`Geocoder fallback request failed with status ${response.status}`);
    }
...
async function initialise() {
  state.map = createMap();
  attachPanelGestures(topPanel, topPanelToggle, topPanelHandle, 'up');
  attachPanelGestures(legendPanel, legendPanelToggle, legendPanelHandle, 'down');
